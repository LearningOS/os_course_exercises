...
A:针对页表大小存储角度。当整个页表的绝大多数页表项都在使用时，单级页表与多级页表所需要的存储空间没有太大差别；但是目前的OS实现中，内核为用户进程提供的虚拟地址映射只会占据整个虚拟地址空间的一小部分，如果使用单级页表，用于存储页表的内存区域将会比多级页表需要的多很多。这时候多级页表的灵活性得到更好的发挥。
