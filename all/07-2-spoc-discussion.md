...
1. 什么是信号量？它与软件同步方法的区别在什么地方？
 ...
 > 信号量是由操作系统提供的一种协调共享资源访问的方法。信号量是一种抽象数据类，由一个被保护的整形 (sem)变量和P()和V()两个原子操作组成，表示系统资源的数量。
...
（第三类读者写者问题）假定有一个存储资源的访问者分成读者和写者两类，它们对存储资源的访问满足如下约定。
 1. 共享读：多个读者可以同时访问存储资源，执行数据读取操作；
 2. 写写互斥和读写互斥：任何时刻只能有一个写者访问存储资源，执行数据写入操作；其他写者和读者都必须等待该写者完成操作，才能访问该存储资源。
 3. 顺序读写：所有读者和写者按到达顺序，开始存储资源的访问。（有写者等待时，后到达的读者不能先于该写者开始访问存储资源；有读者等待时，后到达的写者或读者都不能先于该读者开始访问存储资源）

请使用C、Rust、GO或python语言用信号量和条件变量两种手段在Linux、Windows或MacOS上分别实现第三类读者写者问题。要求先给出自己的实现，然后理解和测试已有的实现，并据此对自己的实现进行可能的改进。
